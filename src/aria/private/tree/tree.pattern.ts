...
import {KeyboardEventManager, Modifier, PointerEventManager} from '../behaviors/event-manager';
...
import {SelectOptions, TreeInputs} from './tree.types';
...
export class TreePattern<V> implements TreeInputs<V> {
    ...
    if (this.inputs.multi() && this.followFocus()) {
      manager
        ...
        .on([Modifier.Ctrl, Modifier.Meta], 'A', () => {
          list.toggleAll();
          list.select(); // Ensure the current item remains selected.
        });
    }
  ...
  /**
   * Sets the tree to its default initial state.
   *
  ...
  expandSiblings(item?: TreeItemPattern<V>) {
    ...
    siblings?.forEach((item: ExpansionItem) => this.expansionBehavior.open(item));
  }
